...
CAMERA_SENSOR_WIDTH_DIRECTORY = os.path.split(os.path.abspath(__file__))[0]
    ...
    def __init__(self):
        self.steps_data=[
            ...
            [   "Compute features",
                ...
                ["-i", "%matches_dir%/sfm_data.json", "-o", "%matches_dir%","-n", "4"] ],
            [   "Compute matches",
            ...
            [   "Densify point cloud",
                ...
                ["--input-file", "%mvs_dir%/scene.mvs", "--resolution-level","0"]],
    ...
    def apply_conf(self, conf):
        ...
        for s in self.steps_data :
            ...
            for o in s[2]:
                ...
                co=co.replace("%logs_dir%",conf.logs_dir)
...
def cd(newdir):
    """
    Change the python working directory to newdir
    """      
    prevdir = os.getcwd()
...
conf.logs_dir = os.path.join(conf.output_dir, "logs")
...
mkdir_ine(conf.logs_dir)
steps.apply_conf(conf)

## WALK
with cd(conf.logs_dir):
